...
public class Handler extends Thread{
    ...
    public void run() {
        try {
            ...
            synchronized (HomeWorkServer.names) {
                if (!HomeWorkServer.names.contains(name)) {
                    HomeWorkServer.names.add(name);
            ...
            HomeWorkServer.writers.add(out);
            ...
            if (name != null) {
                HomeWorkServer.names.remove(name);
            ...
            if (out != null) {
                HomeWorkServer.writers.remove(out);
            }
            try {
                socket.close();
            } catch (IOException e) {
    ...
    public int broadCastMessage(String message) {
        ...
        int counter = 0;
        try {
            for (PrintWriter writer : HomeWorkServer.writers) {
